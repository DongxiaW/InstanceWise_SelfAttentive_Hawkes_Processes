...
for i in range(len(datasets)):
    ...
    plt.figure()
    data = df.query(f'dataset == "{dataset}" and metric=="spearman_rho"')
    ...
    ax.set_xlabel("")
    ax.set_ylabel("Spearman rank-order correlation coefficient")
    ...
    savefig(
        ax.get_figure(), osp.join("data/output", dataset, f"{dataset}-spearman_rho-Tran_final.pdf")
    )
