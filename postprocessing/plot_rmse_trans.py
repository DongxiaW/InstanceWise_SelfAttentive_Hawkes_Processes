...
for i in range(len(datasets)):
    ...
    plt.figure()
    data = df.query(f'dataset == "{dataset}" and metric=="rmse"')
    ...
    ax = sns.barplot(
        ...
        y="value",
        data=data,
        order=["HExp", "RPPN", renaming["ERPP"], "Tran"],
        capsize=0.005,
    )
    ...
    savefig(
        ax.get_figure(), osp.join("data/output", dataset, f"{dataset}-rmse-Tran_final.pdf")
    )
