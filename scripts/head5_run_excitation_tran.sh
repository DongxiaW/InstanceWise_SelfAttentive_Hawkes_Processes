...
if [[ $* == *all* ]] || [[ $* == *Tran* ]]; then
    printf "%s\n" "$WS/tasks/train_tran.py Tran $shared_args --tran_layer 1 --tran_head 5 --cuda --split_id "{0..4} | $LOCAL_RUN
    printf "%s\n" "$WS/tasks/train_tran.py Tran $shared_args --tran_layer 2 --tran_head 5 --cuda --split_id "{0..4} | $LOCAL_RUN
    printf "%s\n" "$WS/tasks/train_tran.py Tran $shared_args --tran_layer 3 --tran_head 5 --cuda --split_id "{0..4} | $LOCAL_RUN
fi
