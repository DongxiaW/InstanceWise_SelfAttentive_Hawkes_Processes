...
class ExplainableTransformerPointProcess(nn.Module):
    def __init__(
        self,
        n_types: int,
        max_mean: float,
    ...
    def forward(
        self, event_seqs, onehot=False, need_weights=True, target_type=-1, feat_mask=None, device=None
    ):
        """[summary]
        ...
        history_emb = self.transformer(feat, feat_mask)
        history_emb = self.linear_layer(history_emb)
        history_emb = history_emb.transpose(1,0)
        ...
        **kwargs,
        ...
        for batch in train_dataloader:
            ...
            log_intensities, log_basis_weights = self.forward(
                batch, need_weights=True, device=device
            )
            nll = self._eval_nll(
    ...
    def predict_next_event(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                _, log_basis_weights = self.forward(batch, need_weights=True, device=device)
        ...
        **kwargs,
        ...
        def func(X, target_type):
            _, log_basis_weights = self.forward(
                X, onehot=True, need_weights=True, target_type=target_type, device=device
            )
