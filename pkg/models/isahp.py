...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.):
        ...
        # )
        self.alpha_layer = nn.Sequential(
            nn.Linear(int(self.d_k*self.h/2), self.d_k, bias=True) #check bias
    ...
    def train_epoch(
        ...
        **kwargs,
        ...
        for batch in train_dataloader:
            ...
            # print('src_mask', reg_src_mask.shape)
            reg_type_mask = F.one_hot(batch[:, :-1, 1].long(), self.n_types).bool().unsqueeze(1)
            torch.set_printoptions(threshold=10000,edgeitems=100)
            # print('type_mask',reg_type_mask.shape, batch[0, :-1, 1].long())
            # trial = reg_src_mask * reg_type_mask
            ...
            # assert not torch.isnan(type_reg_score).any(), f"type_reg_score {type_reg_score}"
            # if torch.any(torch.isnan(type_reg_score)):
            #      print('exist nan')
            # assert not torch.isnan(type_reg_score).any()
            ...
            # print('type_reg_score',type_reg_score.shape, 'type_reg_mask', type_reg_mask.shape)
            for i in range(type_reg_mask.size(-1)):
                ...
                # print('masked_score_array', masked_score_array.shape)
                # print('masked_score_array.nelement()', masked_score_array.nelement())
                if masked_score_array.nelement() > self.n_types:
                    # masked_score_array[masked_score_array != masked_score_array] = 0
                    # history_group_mean = masked_score_array.mean(-1).unsqueeze(-1)
                    # history_grouptype_reg = (masked_score_array - history_group_mean).pow(1).sum(-1)
                    history_group_mean = torch.mean(masked_score_array,-1)
                    history_grouptype_reg = torch.var(masked_score_array,-1)
                    # history_grouptype_reg = (masked_score_array - history_group_mean).pow(2).sum(-1).sqrt()
                    
                    # print('masked_score_array',masked_score_array.shape)
                    # assert not torch.isnan(masked_score_array).any(), f"masked_score_array {masked_score_array}"

                    # print('history_group_mean', history_group_mean, history_group_mean.shape)
            ...
            # print('grouptype_reg', grouptype_reg)
    ...
    def predict_next_event_type(self, dataloader, device=None):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                # print('batch[:, 1:, 1].long()',batch[:, 1:, 1].long().masked_select(mask).shape)
        ...
        **kwargs,
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                # type_mask_j_repeat = torch.repeat_interleave(type_mask_j.unsqueeze(1),T,1) #b, l_i, l_j, k
                # type_mask_i_org = batch[:, 1:, 1].long().unsqueeze(-1) #b, l_i, 1
        ...
        print()
        print('type j', batch[:, :-1, 1].long().detach().cpu())
        print()
        print('type i', batch[:, 1:, 1].long().detach().cpu())
        print()
        ...
        **kwargs,
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                # type_counts += torch.sum(count_agg, 0)  #k,k
        print()
        type_j = batch[:, :-1, 1].long().detach().cpu()
        print('type j', type_j)
        print()
        type_i = batch[:, 1:, 1].long().detach().cpu()
        print('type i', type_i)
        print()
        print('v_score_instance', v_score_instance)
        print()
